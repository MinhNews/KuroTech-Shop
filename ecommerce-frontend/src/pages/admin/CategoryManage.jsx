    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center surface-card p-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Quản lý các danh mục sản phẩm</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-200">
                <th className="p-4 font-medium w-1/4">Tên danh mục</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                <tr><td colSpan="3" className="p-8 text-center text-slate-500">Chưa có danh mục nào.</td></tr>
              ) : (
                filteredCategories.map(cat => (
                  <tr key={cat._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-4 font-medium text-slate-800">{cat.name}</td>
                    <td className="p-4 text-sm text-slate-600 line-clamp-1">{cat.description || '---'}</td>
        ...
        <div className="fixed inset-0 bg-ink/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="surface-card shadow-lift w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <form id="categoryForm" onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div>
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              ...
              <button type="submit" form="categoryForm" className="px-5 py-2.5 bg-slate-900 text-white rounded-lg hover:bg-slate-800 font-medium transition-colors shadow-sm">
                Lưu Danh Mục
              </button>
            </div>
          </div>
